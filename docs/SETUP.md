...
```bash
tar -xzf song-request-linux-0.1.20-linux-x86_64.tar.gz
cd song-request-linux-0.1.20-linux-x86_64
./scripts/check-runtime-prereqs
./scripts/install-desktop-entry
./scripts/song-request-linux-open
```

O pacote ja traz o app compilado. Rust, Cargo e Git so sao necessarios para instalacao via repositorio. O instalador portatil copia o app para `~/.local/share/song-request-linux/app`.
...
1. Baixe `song-request-linux-0.1.20-windows-x86_64.zip`.
2. Extraia o `.zip`.
