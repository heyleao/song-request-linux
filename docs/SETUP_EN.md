...
```bash
tar -xzf song-request-linux-VERSION-linux-x86_64.tar.gz
cd song-request-linux-VERSION-linux-x86_64
...
1. Download the Windows `.zip` from the release, when available.
2. Extract the `.zip`.
