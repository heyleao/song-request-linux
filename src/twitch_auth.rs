...
pub async fn save_bot_token(config: &AppConfig, input: TwitchTokenInput) -> Result<UiConfigView> {
    ...
    let user_config = UiConfigInput {
        default_provider: Some(current.default_provider),
        youtube_playback: Some(current.youtube_playback),
        pear_base_url: Some(current.pear_base_url),
        spotify_client_id: current.spotify_client_id,
        spotify_fallback_enabled: Some(current.spotify_fallback_enabled),
        queue_persistence_enabled: Some(current.queue_persistence_enabled),
        ...
        youtube_allow_non_music: Some(current.youtube_allow_non_music),
